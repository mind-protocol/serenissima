...
import os # For KINOS_API_KEY and TELEGRAM_BOT_TOKEN
...
def _process_direct_message(
    ...
    try:
        ...
        # Check if receiver is a Telegram user or channel
        if receiver_username.startswith('@') or receiver_username.startswith('TG_'):
            _send_telegram_message(sender, receiver_username, content, message_id, details)
...
def _send_telegram_message(sender: str, receiver: str, content: str, message_id: str, details: Dict = None) -> bool:
    ...
    try:
        ...
        # Handle thread routing for channel messages
        if receiver.startswith('TG_') and details:
            thread_id = details.get('telegram_thread_id')
            if thread_id:
                data['message_thread_id'] = thread_id
                log.info(f"Routing to thread {thread_id} for channel {receiver}")
            
            # Also check for reply_to_message_id if responding to specific message
            reply_to = details.get('telegram_reply_to_message_id')
            if reply_to:
        ...
        if response.status_code == 200:
            log.info(f"Successfully sent message to Telegram {receiver}")
            return True
        else:
            log.error(f"Failed to send Telegram message: {response.status_code} - {response.text}")
            return False
