...
# Import meditation activity creators
from backend.engine.activity_creators.lead_meditation_activity_creator import try_create_lead_meditation_activity
from backend.engine.activity_creators.join_meditation_activity_creator import try_create_join_meditation_activity
...
def _handle_lead_meditation(
    tables: Dict[str, Table], citizen_record: Dict, is_night: bool, resource_defs: Dict, building_type_defs: Dict,
    now_venice_dt: datetime, now_utc_dt: datetime, transport_api_url: str, api_base_url: str,
    citizen_position: Optional[Dict], citizen_custom_id: str, citizen_username: str, citizen_airtable_id: str, citizen_name: str, citizen_position_str: Optional[str],
    citizen_social_class: str
) -> Optional[Dict]:
    """Handles leading meditation sessions for Clero during leisure time."""
    if not is_leisure_time_for_class(citizen_social_class, now_venice_dt):
        return None
    ...
    log.info(f"{LogColors.OKCYAN}[Meditation] {citizen_name}: Checking for meditation leadership opportunity.{LogColors.ENDC}")
    
    # Try to create the lead meditation activity
    activity_record = try_create_lead_meditation_activity(
        tables, citizen_record, citizen_position, resource_defs, building_type_defs,
        now_venice_dt, now_utc_dt, transport_api_url, api_base_url,
        check_only=False
    )
    ...
    if activity_record:
        log.info(f"{LogColors.OKGREEN}[Meditation] {citizen_name}: Creating 'lead_meditation' activity.{LogColors.ENDC}")
    
    return activity_record

def _handle_join_meditation(
    tables: Dict[str, Table], citizen_record: Dict, is_night: bool, resource_defs: Dict, building_type_defs: Dict,
    now_venice_dt: datetime, now_utc_dt: datetime, transport_api_url: str, api_base_url: str,
    citizen_position: Optional[Dict], citizen_custom_id: str, citizen_username: str, citizen_airtable_id: str, citizen_name: str, citizen_position_str: Optional[str],
    citizen_social_class: str
) -> Optional[Dict]:
    """Handles joining meditation sessions during leisure time."""
    if not is_leisure_time_for_class(citizen_social_class, now_venice_dt):
        return None
    ...
    log.info(f"{LogColors.OKCYAN}[Meditation] {citizen_name}: Checking for meditation sessions to join (coherence: {coherence:.2f}).{LogColors.ENDC}")
    
    # Try to create the join meditation activity
    activity_record = try_create_join_meditation_activity(
        tables, citizen_record, citizen_position, resource_defs, building_type_defs,
        now_venice_dt, now_utc_dt, transport_api_url, api_base_url,
        check_only=False
    )
    ...
    if activity_record:
        log.info(f"{LogColors.OKGREEN}[Meditation] {citizen_name}: Creating 'join_meditation' activity.{LogColors.ENDC}")
...
def _try_process_weighted_leisure_activities(
    ...
    base_weights = {
        ...
        "Lead Meditation": 15,  # Only for Clero
        "Join Meditation": 10,  # For all citizens
    }
    ...
    class_weight_modifiers = {
        ...
        "Clero": {  # Clergy - religious, educated, moral
            ...
            "Lead Meditation": 40,  # Primary duty - leading group consciousness sessions
            "Join Meditation": 15,  # Sometimes join others' sessions
        },
    ...
    leisure_activities = [
        ...
        (weights["Lead Meditation"], _handle_lead_meditation, "Lead Meditation", True),
        (weights["Join Meditation"], _handle_join_meditation, "Join Meditation", True),
