...
def _handle_professional_construction_work(
    ...
    try:
        ...
        # Get the target building record (we need it for the activity creator)
        target_building = get_building_record(tables, target_building_id)
        if not target_building:
            log.error(f"[Construction] {citizen_name}: Could not find building {target_building_id}")
            return None
            
        # Calculate work duration (default 60 minutes)
        work_duration = 60
        
        # Get path data if citizen is not at the building
        path_data = None
        if citizen_position and target_building['fields'].get('Position'):
            building_pos = json.loads(target_building['fields']['Position'])
            distance_to_site = _calculate_distance_meters(
                (citizen_position['lat'], citizen_position['lng']),
                (building_pos['lat'], building_pos['lng'])
            )
            if distance_to_site > 10:  # If more than 10 meters away
                path_data = get_path_between_points(
                    citizen_position, building_pos, 
                    transport_api_url, citizen_username
                )
        
        # Create construction activity
        activity_record = try_create_construct_building_activity(
            tables, citizen_record, target_building,
            work_duration, contract_id, path_data, now_utc_dt
        )
...
def _handle_occupant_self_construction(
    ...
    try:
        ...
        for project in building_projects:
            ...
            # Calculate work duration based on remaining construction time
            work_duration = min(project['fields'].get('ConstructionMinutesRemaining', 60), 60)
            
            # Get path data if citizen is not at the building
            path_data = None
            if citizen_position and project['fields'].get('Position'):
                building_pos = json.loads(project['fields']['Position'])
                distance = _calculate_distance_meters(
                    (citizen_position['lat'], citizen_position['lng']),
                    (building_pos['lat'], building_pos['lng'])
                )
                if distance > 10:  # If more than 10 meters away
                    path_data = get_path_between_points(
                        citizen_position, building_pos, 
                        transport_api_url, citizen_username
                    )
