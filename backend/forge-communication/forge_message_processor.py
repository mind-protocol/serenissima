...
class ForgeMessageProcessor:
    ...
    def __init__(self):
        # Simple path resolution for production environment
        # The script is in /opt/render/project/src/backend/forge-communication/
        try:
            # Get the directory where this script is located
            script_dir = os.path.dirname(os.path.abspath(__file__))
            
            # The forge-communication directory is the current directory
            self.forge_dir = Path(script_dir)
            
            # CLAUDE.md is in the parent directory (backend)
            backend_dir = os.path.dirname(script_dir)
            self.claude_md_path = Path(backend_dir) / "CLAUDE.md"
            
            # Processed messages file in the same directory as the script
            self.processed_messages_file = self.forge_dir / ".processed_messages.json"
            
            print(f"[DEBUG] Initialized paths - forge_dir: {self.forge_dir}, claude_md: {self.claude_md_path}")
            
        except Exception as e:
            print(f"[ERROR] Failed to initialize paths: {e}")
            # Fallback to simple paths
            self.forge_dir = Path(".")
            self.claude_md_path = Path("../CLAUDE.md")
    ...
    def _get_message_files(self) -> List[Path]:
        ...
        try:
            if not self.forge_dir.exists():
                print(f"Warning: Forge directory {self.forge_dir} does not exist")
                return []
            pattern = str(self.forge_dir / "*.md")
            files = [Path(f) for f in glob.glob(pattern)]
            return [f for f in files if f.name != "README.md"]
        except Exception as e:
            print(f"Error getting message files from {self.forge_dir}: {e}")
            return []
        ...
        try:
            if self.processed_messages_file.exists():
                try:
                    ...
                    print(f"Warning: Could not load processed messages from {self.processed_messages_file}: {e}")
                    return {}
        except Exception as e:
            # Path.exists() might fail if parent directory doesn't exist or permission denied
            print(f"Warning: Could not check if {self.processed_messages_file} exists: {e}")
        return {}
    ...
    def _save_processed_messages(self, processed: Dict):
        ...
        try:
            ...
            print(f"Warning: Could not save processed messages to {self.processed_messages_file}: {e}")
            # Continue without saving - messages will be reprocessed next time
    ...
    def _update_system_prompt(self, new_messages: List[Dict], total_count: int):
        ...
        try:
            ...
            print(f"Warning: Could not update CLAUDE.md at {self.claude_md_path}: {e}")
            # Continue without updating - system will function without latest forge info
...
def run_forge_message_check():
    ...
    # Log to stdout for production environment
    # File logging removed due to permission issues in production
    print(f"[LOG] {datetime.now().isoformat()} - {json.dumps(results)}")
