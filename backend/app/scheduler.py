...
def run_scheduled_tasks(forced_hour: Optional[int] = None): # Added forced_hour parameter
    ...
    while True:
        ...
        if current_minute_venice == 0:
            ...
            # Add pattern feed generation to run every hour at minute 0 and 30
            for hour in range(24):
                # Run at minute 0
                task_name_0 = f"Pattern Feed Generation ({hour:02d}:00 VT)"
                pattern_task_0 = ("../angels/pattern-angel/generate_ultimate_pattern_feed.py", task_name_0, 0)
                if hour in tasks:
                    if not any(t[0] == "../angels/pattern-angel/generate_ultimate_pattern_feed.py" and t[2] == 0 for t in tasks[hour]):
                        tasks[hour].append(pattern_task_0)
                else:
                    tasks[hour] = [pattern_task_0]
                
                # Run at minute 30 (after welfare monitoring)
                task_name_30 = f"Pattern Feed Generation ({hour:02d}:30 VT)"
                pattern_task_30 = ("../angels/pattern-angel/generate_ultimate_pattern_feed.py", task_name_30, 30)
                if not any(t[0] == "../angels/pattern-angel/generate_ultimate_pattern_feed.py" and t[2] == 30 for t in tasks[hour]):
                    tasks[hour].append(pattern_task_30)
