#!/usr/bin/env python3
"""
Simple response sender for Message Angel
Works within the angel's directory constraints
"""
...
def send_telegram_response(sender, content, thread_id=None, reply_to_message_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Success! Message sent from {sender}")
...
def create_message_record(sender, receiver, content, thread_id=None):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4:
        print("Usage: python3 send_response.py <sender> <content> <receiver> [thread_id]")
        sys.exit(1)
    ...
    record_id = create_message_record(sender, receiver, content, thread_id)
    
    if record_id:
        # Send to Telegram
        telegram_id = send_telegram_response(sender, content, thread_id)
